# Рассказы о Ромке и его бабушке · Stories About Romka and His Grandmother
...
По рассказам цикла София Агачер также написала пьесы для детских театральных коллективов:
сборник пьес [«Живая книга»](https://ridero.ru/books/sbornik_pes_zhivaya_kniga/), а ежегодно
по этим пьесам проходит фестиваль [«Живая книга»](https://festival-zhivaya-kniga.vercel.app/).
Оба ресурса выведены в отдельный блок лендинга, в ссылки футера и в микроразметку Schema.org.
...
| 3.2 О цикле | Дословные тексты RU/EN из ТЗ, ссылка на <https://sofia-agacher.vercel.app/>, состав цикла, галерея иллюстраций-разворотов страниц. |
| 3.2+ Пьесы и фестиваль | Блок «Пьесы и фестиваль» (`#plays`): пьесы Софии Агачер для детских театральных коллективов, сборник [«Живая книга»](https://ridero.ru/books/sbornik_pes_zhivaya_kniga/) на Ridero и ежегодный [фестиваль «Живая книга»](https://festival-zhivaya-kniga.vercel.app/). Ссылки — в навигации, в футере и в JSON-LD (`Book` + `Festival`). |
...
| 3.5 Футер | Автор, ссылки на сборник пьес и фестиваль, контакты, юридические документы, копирайт. |
...
| 8 Доп. требования | SEO-метаданные на двух языках, Schema.org `BookSeries` + `Book` + `AudioObject` + `Festival`, robots/sitemap, HSTS и security-заголовки, политика конфиденциальности и согласие на обработку данных. Показ иллюстраций страниц — картинками в разделе «О цикле». |
...
src/
  app/
    ...
    page.tsx                # лендинг + JSON-LD (BookSeries/Book/AudioObject/Festival)
  ...
  components/               # Header, Hero, About, Plays, StoriesGrid, LeadForm, Footer, LanguageSwitcher, admin/*
  lib/
...
### 8.1 Галерея иллюстраций-разворотов

В разделе «О цикле» три разворота книги (1615×808) показываются слайдером на всю
ширину контейнера. По клику открывается лайтбокс.

Важная деталь: в лайтбоксе используется обычный `<img>` с исходным файлом, а не
`next/image`. Оптимизатор Next пережимает JPEG (по умолчанию `q=75`) и меняет
масштаб, из-за чего мелкий двуязычный текст на развороте становится мыльным.
Поэтому в лайтбоксе файл отдаётся как есть, в натуральных пикселях.

Картинка открывается в реальном размере (1:1) и позиционируется абсолютно по
центру (`margin = −половина натурального размера`), чтобы увеличение не «съедало»
края, как это бывает при flex-центрировании. Управление:

| Действие | Результат |
| --- | --- |
| Колесо мыши | Масштаб от «вписано» до 3×. |
| Наведение на край окна | Панорамирование: разворот едет за курсором. В центральной зоне (55 %) картинка неподвижна, чтобы текст не «плыл». |
| Перетаскивание | Сдвиг картинки с ограничением по краям. |
| Кнопка «Вписать целиком» | Разворот целиком в окне. |
| Кнопка «Реальный размер» | Возврат к 1:1. |
| Двойной клик | Переключение «вписано» ↔ 1:1. |
| `+` / `−` | Масштаб. |
| `0` / `1` | Вписать / реальный размер. |
| `←` / `→` | Предыдущий / следующий разворот. |
| `Esc`, крестик | Закрыть просмотр. |

Сдвиг всегда ограничен так, чтобы любой край разворота был достижим, но картинку
нельзя было утащить за пределы окна и «потерять».
